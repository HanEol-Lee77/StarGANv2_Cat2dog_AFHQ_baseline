...
def calculate_metrics(nets, args, step, mode):
    ...
    domains = os.listdir(args.val_img_dir) # testA, testB
    domains.sort()
    ...
    for trg_idx, trg_domain in enumerate(domains):
        ...
        for src_idx, src_domain in enumerate(src_domains):
            path_src = os.path.join(args.val_img_dir, src_domain) # val_img_src_dir 아님!
                ...
                for j in range(args.num_outs_per_domain):
                    if mode == 'latent':
                        ...
                        try:
                            ...
                            # print(f'next(iter_ref) : {next(iter_ref)}')
...
def calculate_fid_for_all_tasks(args, domains, step, mode):
    ...
    for trg_domain in domains:
        src_domains = [x for x in domains if x != trg_domain]
        print('testsetsetst ',src_domains)
        trg_domain_train=''
        for src_domain in src_domains:
            task = '%s2%s' % (src_domain, trg_domain)
            print("foxxxxxxxxxxxxx ", trg_domain)
            if 'B' in trg_domain:
                trg_domain_train = 'trainB'
            else:
                trg_domain_train = 'trainA'
            path_real = os.path.join(args.train_img_dir, trg_domain_train)
            path_fake = os.path.join(args.eval_dir, task)
...
#     for trg_domain in domains:
#         src_domains = [x for x in domains if x != trg_domain]
#         print(f'test: {src_domains}')
#         for src_domain in src_domains:
#             task = '%s2%s' % (src_domain, trg_domain)
#             print("foxxxxxxxxxx", trg_domain)
#             if 'B' in trg_domain:
#                 trg_domain_train = 'trainB'
#             else:
#                 trg_domain_train = 'trainA'
